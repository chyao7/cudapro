...
__global__ void nms(Box *boxes, int *keep, int box_num, float nms_threshold) {
    ...
    for (int i = 0; i < blockDim.x; ++i) {
        ...
        float inter_x1 = max(s_boxes[threadIdx.x].x1, s_boxes[i].x1);
        float inter_y1 = max(s_boxes[threadIdx.x].y1, s_boxes[i].y1);
        float inter_x2 = min(s_boxes[threadIdx.x].x2, s_boxes[i].x2);
        float inter_y2 = min(s_boxes[threadIdx.x].y2, s_boxes[i].y2);

        float inter_area = max(inter_x2 - inter_x1 + 1, 0.0f) * max(inter_y2 - inter_y1 + 1, 0.0f);

        float box_area1 = (s_boxes[threadIdx.x].x2 - s_boxes[threadIdx.x].x1 + 1) * (s_boxes[threadIdx.x].y2 - s_boxes[threadIdx.x].y1 + 1);
        float box_area2 = (s_boxes[i].x2 - s_boxes[i].x1 + 1) * (s_boxes[i].y2 - s_boxes[i].y1 + 1);
